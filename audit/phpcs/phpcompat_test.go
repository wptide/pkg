...
func TestPhpCompat_summaryPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotFilename, gotPath, err := p.reportPath(tt.args.r, tt.args.fileSuffix)
			if (err != nil) != tt.wantErr {
				t.Errorf("PhpCompat.reportPath() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotFilename != tt.wantFilename {
				t.Errorf("PhpCompat.reportPath() gotFilename = %v, want %v", gotFilename, tt.wantFilename)
			}
			if gotPath != tt.wantPath {
				t.Errorf("PhpCompat.reportPath() gotPath = %v, want %v", gotPath, tt.wantPath)
			}
		})
	}
}
